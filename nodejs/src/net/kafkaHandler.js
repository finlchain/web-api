...
//
let myBrokerList = "192.168.10.220:9092";
// let myBrokerList = "192.168.30.220:9092";
let myTopicList = "253f7f036004";
let mySubNetId;

//
module.exports.setMyKafkaInfo = (brokerList, topicList) => {
    myBrokerList = brokerList;
    myTopicList = topicList;
}

//
module.exports.setMyBrokerList = (brokerList) => {
    myBrokerList = brokerList;
}

//
module.exports.getMyBrokerList = () => {
    return myBrokerList;
}

//
module.exports.setMyTopicList = (topicList) => {
    myTopicList = topicList;
}

//
module.exports.getMyTopicList = () => {
    return myTopicList;
}
...
/////////////////////////
module.exports.sendContractMsg = async (msg) => {
    let ret_val = false;

    let netInfo = await dbISHandler.getSystemInfoBgStatus();

    if (netInfo[0].bg_status === define.DATA_HANDLER.status_cmd.start)
    {
        let contractJson = JSON.stringify(msg);

        this.sendRdKafkaMsgStd(this.getMyBrokerList(), this.getMyTopicList(), contractJson);

        ret_val = true;
    }
    else
    {
        logger.error("Failed to send Contract Message");
    }

    return ret_val;
}

module.exports.sendContractJsonMsg = async (contractJson) => {
    this.sendRdKafkaMsgStd(this.getMyBrokerList(), this.getMyTopicList(), contractJson);
}
...
module.exports.sendRdKafkaMsgStd = async (brokers, topicName, msg) => {
    ...
    producer.on('ready', function(arg) {
        try {
            ...
            // this.sendRdKafkaMsgStd2(producer, brokers, topicName, msg);
...
//
module.exports.sendRdKafkaMsgStd2 = (producer, brokers, topicName, msg) => {
    // logger.debug("brokers : " + brokers);
    logger.debug("topicName : " + topicName);
